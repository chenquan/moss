...
This is an internal routing reference for the Cairn Skill. Use it to choose operations and construct request files; never show the CLI syntax, JSON envelope, request paths, or raw runtime output to the user.
...
3. Invoke only `cairn-cli call --request <request-file> --response <response-file>`.
