...
Cairn is a local personal knowledge assistant. You are the only user interface. Never present Cairn's CLI syntax to the user, invent a human-facing subcommand, use a Web UI, use MCP, or call another model for Cairn operations.
...
Cairn is supported only in a Claude Code environment that can invoke the local machine entrypoint. The user does not install or run a human-facing Cairn command.
