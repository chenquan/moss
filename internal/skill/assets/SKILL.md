...
- Claude may write only Moss-issued compile staging/result files. Never directly edit SQLite, the final Wiki, indexes, plans, trash, backups, or audit records.
...
- On `ok: false`, report the stable error code in plain language and retry only when `retryable` is true. A non-zero process exit or missing stdout response is a runtime failure, not a business success.
...
2. Construct a stdin request containing `source.ingest`, the input file path, `source_type`, `sensitivity`, and an idempotency key. Use a quoted heredoc or equivalent; do not interpolate the document contents.
3. Invoke `moss call` once and consume the single structured stdout response.
...
4. Write the stage JSON to the exact managed result path and send a stdin `compile.submit` request containing only that path. Follow the returned order `extract` → `classify` → `write`; do not skip, replay, or edit a different stage file.
...
For installation, upgrade, repair, or explicit health questions, call the relevant maintenance operations through stdin/stdout. Do not run a handshake before every ordinary workflow. Explain component failures plainly, but do not expose raw command output or require the user to run CLI commands.
...
2. Initialize the data root by invoking `system.handshake` and `system.health` through stdin/stdout. Opening the runtime creates the private SQLite/database, Raw, Wiki, Jobs, backup, trash, lock, response, and staging directories when absent.
...
1. Handshake with the currently installed runtime and call `system.export` through stdin/stdout before replacing the binary or Skill. Keep the returned private backup path as the recovery reference; treat the archive as sensitive.
2. Install the trusted matching binary/Skill resources as a pair, then run `system.handshake` and `system.health` as a migration preflight. Do not write user data while the preflight is failing.
3. If the new runtime is healthy and protocol-compatible, report the version and backup reference. If it is incompatible or unhealthy, restore or retain the previous matching pair, stop, and explain that no normal writes should continue. Never fall back to transport flags.
