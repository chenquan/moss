...
The Skill SHALL establish protocol compatibility during installation, upgrade, repair, or an explicit maintenance workflow, SHALL invoke normal operations directly after a compatible installation is established, and SHALL stop with an actionable compatibility message when the CLI is missing or incompatible.
...
- **WHEN** a stdio call returns `PROTOCOL_VERSION_UNSUPPORTED` or the installed binary cannot parse the stdio entrypoint
- **THEN** the Skill stops the workflow, reports that the matching Moss binary and Skill must be upgraded together, and does not silently create a temporary file transport fallback
...
- **WHEN** the installed binary does not support the stdio `call` entrypoint
...
### Requirement: Maintenance preamble is not repeated per turn
The Skill SHALL not invoke handshake, health, or capabilities before every normal user operation once a compatible installation has been established.

#### Scenario: Normal operation after installation
- **WHEN** the Skill has already established a compatible Moss installation and the user requests capture, retrieval, or an action
- **THEN** the Skill invokes the target operation directly through stdin/stdout

#### Scenario: Maintenance failure trigger
- **WHEN** a normal call reports a missing binary, protocol incompatibility, storage failure, or another runtime condition requiring repair
- **THEN** the Skill enters the maintenance path, performs the relevant handshake/health checks, and does not repeat the failing business call indefinitely
